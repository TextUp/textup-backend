...
class RecordItems {
    ...
    static Result<? extends RecordItem> mustFindModifiableForId(Long thisId) {
        RecordItems.mustFindForId(thisId)
            .then { RecordItem rItem1 ->
                if (rItem1.instanceOf(RecordNote) && (rItem1 as RecordNote).isReadOnly) {
                    IOCUtils.resultFactory.failWithCodeAndStatus("recordItems.cannotModify",
                    ResultStatus.FORBIDDEN, [thisId])
                }
